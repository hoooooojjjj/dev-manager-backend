...
# .env.prod 파일의 환경 변수를 fly.io secrets로 자동 설정
...
# .env.prod 파일 존재 확인
if [ ! -f .env.prod ]; then
  echo "❌ 오류: .env.prod 파일을 찾을 수 없습니다."
  echo "💡 .env.example을 복사하여 .env.prod 파일을 생성하고 배포 환경 값을 채워주세요."
  exit 1
fi
...
echo ""
echo "📋 .env.prod 파일에서 환경 변수를 읽어 fly.io secrets에 설정합니다..."
echo ""

# .env.prod 파일 읽기 및 secrets 설정
